...
class SynRotator(object):
    ...
    def rotate(self, pred_syn, vol_im, bboxes):
        ...
        if os.path.exists(save_txt % (bid)) or os.path.exists(
            save_pref % (bid) + "_rot.txt"
        ):
            continue
        ...
        # storage
        syn = np.zeros(
            (self.pad_sz * 2 + 1, self.pad_sz * 2 + 1), dtype=np.uint8
        )
        im = 128 * np.ones(
            (self.pad_sz * 2 + 1, self.pad_sz * 2 + 1), dtype=np.uint8
        )
        dilation_mask = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, (self.thres_dilation, self.thres_dilation)
        )
        ...
        for bid in range(bboxes.shape[0]):
            ...
            if os.path.exists(save_txt % (bid)) or os.path.exists(
                ...
                if (
                    np.max(np.sum(pred_pre.reshape(pred_sz[0], -1), axis=1))
                    < self.thres_slice
                    or np.max(
                        np.sum(pred_post.reshape(pred_sz[0], -1), axis=1)
                    )
                    < self.thres_slice
                ):
                    # bboxesox: total num
                    ...
                    if (
                        np.max(
                            np.sum(
                                (pred_pre * mm_max).reshape(pred_sz[0], -1),
                                axis=1,
                            )
                        )
                        < self.thres_slice
                        or np.max(
                            np.sum(
                                (pred_post * mm_max).reshape(pred_sz[0], -1),
                                axis=1,
                            )
                        )
                        < self.thres_slice
                ...
                if not do_syn:
                    # no synapse output
                    np.savetxt(
                        save_txt % bid, [pred_pre.sum(), pred_post.sum()], "%d"
                    )
                    ...
                    zsum = np.sum(
                        ((pred_pre + pred_post) * mm_max).reshape(
                            pred_sz[0], -1
                        ),
                        axis=1,
                    )
                    ...
                    pred_pre_max = (pred_pre[zid] * mm_max[zid]).astype(
                        np.uint8
                    ) * 128
                    pred_post_max = (pred_post[zid] * mm_max[zid]).astype(
                        np.uint8
                    ) * 255
                    ...
                    # can exceed pad_sz
                    sz2 = syn[
                        max(0, py_out) : max(0, py_out) + pred_sz[1],
                    ...
                    if np.sum(pred_pre_max > 0) > np.sum(pred_post_max > 0):
                        ...
                        syn_patch[pred_post_max > 0] = pred_post_max[
                            pred_post_max > 0
                        ]
                    else:
                        syn_patch = pred_post_max
                        syn_patch[pred_pre_max > 0] = pred_pre_max[
                            pred_pre_max > 0
                        ]
                    ...
                    syn[
                        max(0, py_out) : max(0, py_out) + tmp.shape[0],
                        max(0, px_out) : max(0, px_out) + tmp.shape[1],
                    ] = syn_patch
                    # 2. output image
                    # load image
                    im[:] = 128
                    ...
                    im_patch = im[
                        pz,
                        max(0, py - pad_sz) : py + pad_sz + 1,
                        max(0, px - pad_sz) : px + pad_sz + 1,
                    ]
                    im[
                        max(0, pad_sz - py) : max(0, pad_sz - py)
                        + im_patch.shape[0],
                        max(0, pad_sz - px) : max(0, pad_sz - px)
                        + im_patch.shape[1],
                    ] = im_patch
                    ...
                    cleft = np.logical_and(
                        cv2.dilate(
                            (syn == 255).astype(np.uint8), dilation_mask
                        ),
                        cv2.dilate(
                            (syn == 128).astype(np.uint8), dilation_mask
                        ),
                    )
                    if cleft.max() == 0:
                        cleft = syn > 0
                    ...
                    # re-center
                    imsave(
                        save_pref % bid + "_im.png",
                        rotateIm(im, -angle, tuple(pt_m)),
                    )
                    imsave(
                        save_pref % bid + "_syn.png",
                        rotateIm(syn, -angle, tuple(pt_m)),
                    )
                    np.savetxt(
                        save_pref % bid + "_rot.txt",
                        [-angle] + list(pt_m),
